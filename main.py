...
with mp_pose.Pose(min_detection_confidence=0.7, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        try:
            ...
            angle_1 = angle(right_shoulder, right_elbow, right_wrist)
            angle_2 = angle(left_shoulder, left_elbow, left_wrist)

            # Drawing the angle onto the feed
            cv.putText(image, str(angle_1), tuple(np.multiply(right_elbow, [640, 480]).astype(
                int)), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2, cv.LINE_AA)
            cv.putText(image, str(angle_2), tuple(np.multiply(left_elbow, [640, 480]).astype(
                int)), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2, cv.LINE_AA)

            if angle_1 > 160 and angle_2 > 160:
                position = 'down'
            if angle_1 < 30 and angle_2 < 30 and position == 'down':
                position = 'up'
                counter += 1
